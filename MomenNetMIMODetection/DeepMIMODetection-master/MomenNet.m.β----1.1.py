...
for i in range(1,L):
    ...
    ZZ = relu_layer(Z, 4*K + v_size , hl_size,'relu'+str(i))
    S.append(sign_layer(ZZ , hl_size , K,'sign'+str(i)))
    ##S[i]=(1-res_alpha)*S[i]+res_alpha*S[i-1]               #MomentNet.M删除部分
    S_new = S[-1]                 # 网络输出                  #MomentNet.M补充部分
    G = S_new - S[i-1]            # “更新方向”                #MomentNet.M补充部分
    M.append(beta[i] * M[i-1] + (1 - beta[i]) * G)           #MomentNet.M补充部分   #β学习率修改部分
    S[-1] = S[i-1] + M[-1]                                   #MomentNet.M补充部分

    V.append(affine_layer(ZZ , hl_size , v_size,'aff'+str(i)))
    ##V[i]=(1-res_alpha)*V[i]+res_alpha*V[i-1]               #MomentNet.M删除部分  
    if LOG_LOSS == 1:
