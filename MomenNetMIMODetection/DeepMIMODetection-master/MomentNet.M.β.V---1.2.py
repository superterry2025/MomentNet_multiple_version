...
M = []                                     #MomentNet.M补充部分
M.append(tf.zeros([batch_size, K]))        #MomentNet.M补充部分
LOSS=[]
LOSS.append(tf.zeros([]))
BER=[]
BER.append(tf.zeros([]))                                      
beta_raw = [                                                                           #β学习率补充
    tf.Variable(2.0, dtype=tf.float32, trainable=True, name='beta_raw_'+str(i))        #β学习率补充
    for i in range(L)                                                                  #β学习率补充
]
beta = [tf.sigmoid(b) for b in beta_raw]                                               #β学习率补充部分
...
for i in range(1,L):
    ...
    Z = tf.concat([HY, S[-1], M[-1], temp1, V[-1]], 1)       #MomentNet.M修改部分
    ZZ = relu_layer(Z, 4*K + v_size , hl_size,'relu'+str(i)) #MomentNet.M修改部分
    DeltaS = affine_layer(ZZ, hl_size, K, 'delta'+str(i))    #MomentNet.M修改部分
    G = DeltaS                                               #MomentNet.M修改部分
    M.append(beta[i] * M[i-1] + (1 - beta[i]) * G)           #MomentNet.M修改部分
    S.append(S[i-1] + M[-1])                                 #MomentNet.M修改部分
    V.append(                                                #V稳定性修改部分
    (1 - res_alpha) * tf.nn.relu(                            #V稳定性修改部分
        affine_layer(ZZ, hl_size, v_size, 'aff'+str(i))      #V稳定性修改部分
    ) 
    + res_alpha * V[-1])                                     #V稳定性修改部分
...
train_vars = tf.trainable_variables()                                               #β学习率补充部分                                        
beta_vars = [v for v in train_vars if 'beta_raw' in v.name]                         #β学习率补充部分
main_vars = [v for v in train_vars if 'beta_raw' not in v.name]                     #β学习率补充部分
train_step_main = tf.train.AdamOptimizer(learning_rate).minimize(                   #β学习率补充部分
    TOTAL_LOSS, var_list=main_vars                                                  #β学习率补充部分
)                                               
train_step_beta = tf.train.AdamOptimizer(learning_rate * 0.01).minimize(            #β学习率补充部分
    TOTAL_LOSS, var_list=beta_vars                                                  #β学习率补充部分
)
...
for i in range(train_iter): #num of train iter
    ...
    sess.run(                                                                           #β学习率补充部分
    [train_step_main, train_step_beta],                                                 #β学习率补充部分
    feed_dict={HY: batch_HY, HH: batch_HH, X: batch_X}                                  #β学习率补充部分
)
